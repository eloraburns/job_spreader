...
!/taavi/job_spreader/blob/master/job_spreader.png(Image showing relationship of dispatcher to feeders to spreader.)!

The dispatcher generates groups of jobs.  Each group contains zero or more jobs.

Each feeder queue asks for a group of jobs from the dispatcher and produces each of the jobs inside one at a time.  When the group it's working on is empty, it asks the dispatcher for another group of jobs.  When the dispatcher has no more groups of jobs to give, the feeder stops producing jobs.

The spreader asks each feeder, in turn, for a job.  It produces each job as it gets them from the feeders.
...
Effectively what we're doing is lazily bin-pack the incoming groups of jobs from the dispatcher into feeder queues.
